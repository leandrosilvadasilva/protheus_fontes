...
Local cMoeda        := '01'
Local cPeriodo		:= '01'
Local nPeriodos		:= mv_par02 - mv_par01
Local lTitFuturo	:= .T.
Local lCtrc			:= .T.
...
If lDMSCompra
	...
	For nX := 1 To Len(aDMSCompras)
        ...
        cMsg := "CI_M180 - 'Processando Pedido de Compra de Máquinas'"
        If !lAuto
            oProcess:IncRegua2(cMsg)   
        Else
            ConOut(cMsg)
...
If lDMSVenda
	...
	For nX := 1 To Len(aDMSVendas)
        ...
        cMsg := "CI_M180 - 'Processando Pedido de Venda de Máquinas'"
        If !lAuto
            oProcess:IncRegua2(cMsg)   
        Else
            ConOut(cMsg)
...
If aArqTmp == Nil .And. aArqco == Nil
	IW_MSGBOX('CI_M180','É necessario escolher ao menos um tipo','STOP') 
Else
...
cQuery += " ('APL','Aplicações'),"
cQuery += " ('EMP','Empréstimos'),"
cQuery += " ('SOF','Solicitações de Fundos'),"
cQuery += " ('DMSC','Compras DMS'),"
cQuery += " ('DMSV','Vendas DMS'),"
cQuery += " ('CTRC','Documentos de Transporte'),"
cQuery += " ('CHQ','Cheques'),"
cQuery += " ('CRI','Inadimplência Contas a Receber'),"
cQuery += " ('CPI','Inadimplência Contas a Pagar')"  
...
While (cAliasTrb)->( !Eof() ) //IndRegua
	IncProc('Processando Comissões') //"Processando Comissäes"
...
While (cAliasTrb)->(!Eof()) //IndRegua
	IncProc('Processando Solicitações de Fundo') //"Processando Solicitações de Fundo"
