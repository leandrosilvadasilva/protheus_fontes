...
if cTipo == '0'//entrada
	cQuery := ""
	cQuery := " SELECT COUNT(*) CAMPO FROM "+RetSqlName('SD1')+" SD1 "
	cQuery += " WHERE "
	cQuery += " 		D1_FILIAL = '"+xFilial("SD1")+"'  "
	cQuery += " 	AND D1_SERIE = '"+cSerie+"' "
	cQuery += " 	AND D1_DOC = '"+cNota+"' "
	cQuery += " 	AND D1_FORNECE = '"+cCliefor+"' AND D1_LOJA = '"+cLoja+"' "
	cQuery += " 	AND SUBSTRING(D1_COD,1,3)='CP-' AND D_E_L_E_T_ = '' "
	if empty(cAutXml)
		cAutXml := iif(SFQuery(cQuery)==0,"","05794609000101")
	else
		cAutXml := iif(SFQuery(cQuery)==0,"",";05794609000101")
	endif
elseif cTipo == '1'//saida
	cQuery := ""
	cQuery := " SELECT COUNT(*) CAMPO FROM "+RetSqlName('SD2')+" SD2 "
	cQuery += " WHERE "
	cQuery += " 		D2_FILIAL = '"+xFilial("SD2")+"'  "
	cQuery += " 	AND D2_SERIE = '"+cSerie+"' "
	cQuery += " 	AND D2_DOC = '"+cNota+"' "
	cQuery += " 	AND D2_CLIENTE = '"+cCliefor+"' AND D2_LOJA = '"+cLoja+"' "
	cQuery += " 	AND SUBSTRING(D2_COD,1,3)='CP-' AND D_E_L_E_T_ = '' "
	if empty(cAutXml)
		cAutXml := iif(SFQuery(cQuery)==0,"","05794609000101")
	else
		cAutXml := iif(SFQuery(cQuery)==0,"",";05794609000101")
...
//IATAN EM 12/09/2023 - CHAMADO #10469
...
//cString += NfeTag('<vOutro>' ,ConvType(949.23,15,2)) // outras despesas - AFRMM
...
Static Function NfeTotal(aTotal,aRet,aICMS,aICMSST,lIcmDevol,cVerAmb,aISSQN,nVicmsDeson,aNota,nVIcmDif,aAgrPis,aAgrCofins,nValLeite )
	...
	//IATAN EM 12/09/2023 CHAMADO #10469
	cString += '<vNF>'+ConvType(aTotal[02]+aTotal[03]+vlAFRMM,15,2)+'</vNF>' // PISST + COFINSST serão somados a vNF caso indSomaPISST = 1/indSomaCOFINSST = 1 NT 2020.005
//	cString += '<despesa>'+ConvType(949.23,15,2)+'</despesa>'
//	cString += '<vNF>'+ConvType(384926.09,15,2)+'</vNF>' // PISST + COFINSST serão somados a vNF caso indSomaPISST = 1/indSomaCOFINSST = 1 NT 2020.005
...
IF len(aDetPag) > 0  
	...
	For nX := 1 To Len(aDetPag)
		...
		If aDetPag[nX][1] == "90" //SEM PAGAMENTO
			...
			//iatan - chamado #10469
			cString +='<valor>'+ConvType(aDetPag[nX][2]+vlAFRMM,15,2)+'</valor>'
//			cString +='<valor>'+ConvType(384926.09,15,2)+'</valor>'
		EndIf
...
Static Function SFQuery( cQuery )
***********************
	Local cRet 
	if (GetRemoteType() != 1)
