      ...
      IF lOk
	   ...
	   cQuery += " GROUP BY A1_COD,A1_LOJA"
		...
		Else
			...
			aAdd( aCliente, {"A1_NOME"   , UPPER(U_USLimpaEsp(oJson:A1_NOME))		  	, Nil} )
			aAdd( aCliente, {"A1_CONTATO", UPPER(U_USLimpaEsp(oJson:A1_CONTATO))	 	, Nil} )		
			aAdd( aCliente, {"A1_NREDUZ" , UPPER(U_USLimpaEsp(oJson:A1_NREDUZ))	  	, Nil} )
			aAdd( aCliente, {"A1_TIPO"   , UPPER(oJson:A1_TIPO)    		, Nil} )
			aAdd( aCliente, {"A1_CEP"    , oJson:A1_CEP         		, Nil} )
			aAdd( aCliente, {"A1_END"    , UPPER(U_USLimpaEsp(oJson:A1_END))	     	, Nil} )
			aAdd( aCliente, {"A1_BAIRRO" , UPPER(U_USLimpaEsp(oJson:A1_BAIRRO))	  	, Nil} )
			...
			if alltrim(UPPER(oJson:A1_PESSOA)) == 'F'
				aAdd( aCliente, {"A1_CLASSE"  , "A"	                		, Nil} )
				aAdd( aCliente, {"A1_RISCO"  , "A"	                		, Nil} )
				aAdd( aCliente, {"A1_MSBLQL" , '2'	    		, Nil} )
				aAdd( aCliente, {"A1_NATUREZ", '111101'		    , Nil} )
				aAdd( aCliente, {"A1_CONTRIB" , '2'	    		, Nil} )
				aAdd( aCliente, {"A1_COND" , '039'    		, Nil} )
				aAdd( aCliente, {"A1_VENCLC" , YearSum( dDataBase ,1) 		, Nil} )
				cZHOCOND := '039'
			else	
...
User Function USLimpaEsp(cTexto)
