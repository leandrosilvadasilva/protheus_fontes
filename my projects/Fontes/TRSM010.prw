...
/* Alteracoes Realizadas desde a Estruturacao Inicial
	...
	01/02/2023					Davidson Carvalho					Arquivos não estão sendo movidos para a pasta FTP Totvs
	08/02/2023					Davidson Carvalho					Inclusão dos Cases solicitado por Tiago Gregroletto.
	13/02/2023                  Davidson Carvalho					Adaptação para consisitr as notas deletadas 
	15/02/2023					Davidosn Carvalho					Analise campo C6_SALDO,fora de orderm no arquivo.
	17/02/2023					Davidson Carvalho					Analise enderço de saida FTP / Endereço do arquivo extração.zip	
	02/03/2023                  Davidson Carvalho					Ajuste de posição solicitado no Ticket 8026 (cStsPv := "Incluido")
	19/04/2023					Davidson Carvalho					Criação de parametros para faciliatar a manutenção de variaveis.
	20/06/2023 					Davidson Carvalho					Erro variavel cAliasT não declarada na função M010AB3Exp.
	01/08/2023     				Davidson Carvalho					9974/EXPORTACAO SALDO FISICO E FINANCEIRO SB2/EXPORTACAO SALDO POR LOTE SB8
	11/12/2023     				Leonor								11926/ RETORNADO EXPORTACAO SALDO FISICO E FINANCEIRO SB2/EXPORTACAO SALDO POR LOTE SB8
*/
User Function TRSM010( aParam )
	...
	Else
		...
		cPerg := PadR( "TRSM010", Len( SX1->X1_GRUPO ), "" )
...
/* INICIO - PARAMETRO EXCLUIDO POR GABRIEL.COSTA EM 19/08/22, SOLICITAÃ‡ÃƒO PEDIDO FABIO */
...
/*
	...
	EndIf
*/
/* FIM - PARAMETRO EXCLUIDO POR GABRIEL.COSTA EM 19/08/22, SOLICITAÃ‡ÃƒO PEDIDO FABIO */
	...
	*/      
	...
	//+------------------------------------------+
	//--Comentado Ticket 9974 Davidson Carvalho // retornado dia 11/12 ticket 11926 Leonor
	If lAuto//Via Schedule
...
If !lAuto
	oProcess:SaveLog( "Fim da Exportação da Tabela de Matriz de reposicao ZA5 " )
EndIf
...
Local nAnoDtValidSBJ := SuperGetMv( "MH_ANOSBJ",.F.,2050 ) // Ano da maior data de validade dos saldo iniciais por lote  SBJ - Saldos Iniciais por Lote
Local cAliasT        := GetNextAlias()
Local cQuery         := ""
Local cMsg           := ""
Local cDtValid       := ""
...
If ( cAliasT )->( !Eof() )
	...
	While ( cAliasT )->( !Eof() )
			...
			If  Year( StoD( ( cAliasT )->BJ_DTVALID ) ) > nAnoDtValidSBJ 
				cDtValid := Alltrim("31/12/"+Str(nAnoDtValidSBJ))
				//cDtValid := "31/12/2050"
			Else
...
If !lAuto
	oProcess:SaveLog("Inicio da Extração da Tabela de Comodatos (Z30)")
	oProcess:SetRegua1( 0 )
Else
...
If !lAuto
	oProcess:SaveLog( "Fim da Exportação da Tabela de Comodatos Z30 " )
EndIf
...
Static Function M010Cliente(oProcess,lAuto)
	...
	// COMENTADO POR IATAN EM 21/05/2024 - A PEDIDO DE GABRIEL DA MA
	//aSldCli()
...
/*
	...
	Data                        Programador                          Motivo
	22/01/2023					Davidson Carvalho					Ajuste C9_ITEM,C6_NOTA,C6_SERIE,C6_DATFAT
*/
...
//Inicio Alterações
//-- Alterado por Davidson Carvalho a pedido do Tiago Gregoletto em 22/01/2023 -  Motivo : Ajuste C6_NOTA e  C6_SERIE.
//	cQuery += "         SC6.C6_NOTA,"
	cQuery += " (SELECT TOP(1) C6_NOTA FROM SC6010 A "
	cQuery += " WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = SC6.C6_FILIAL AND A.C6_NUM = SC6.C6_NUM AND A.C6_PRODUTO = SC6.C6_PRODUTO AND C6_NOTA <> '') C6_NOTA,"

//	cQuery += "         SC6.C6_SERIE,"
	cQuery += " (SELECT TOP(1) C6_SERIE FROM SC6010 A "
	cQuery += " WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = SC6.C6_FILIAL AND A.C6_NUM = SC6.C6_NUM AND A.C6_PRODUTO = SC6.C6_PRODUTO AND C6_NOTA <> '') C6_SERIE,"

//	cQuery += "         SC6.C6_DATFAT,"
	cQuery += " (SELECT TOP(1) C6_DATFAT FROM SC6010 A "
	cQuery += " WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = SC6.C6_FILIAL AND A.C6_NUM = SC6.C6_NUM AND A.C6_PRODUTO = SC6.C6_PRODUTO AND C6_NOTA <> '') C6_DATFAT,"

	cQuery += "         SC6.C6_ITEM,"
	cQuery += "         SC6.C6_PRODUTO,"
	cQuery += "         SC6.C6_UM,"
	cQuery += "         SC6.C6_QTDVEN, " // VEN_ORIG, Davidson Carvalho 08/02/2023
//	cQuery += "         SC6.C6_QTDVEN,"
//	cQuery += "	(SELECT SUM(C6_QTDVEN) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) as C6_QTDVEN,"

//
//	cQuery += "         SC6.C6_QTDENT,"
// 	cQuery += "	(SELECT SUM(C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) as C6_QTDENT,"
	cQuery += "		CASE "
	cQuery += "		WHEN (SELECT SUM(C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO)  = (SELECT SUM(C6_QTDVEN) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO)		"
	cQuery += "		THEN C6_QTDVEN 		"//--COLOCO A mesma quantidade de venda
	cQuery += "		ELSE C6_QTDENT 	"
	cQuery += "		END C6_QTDENT, 	"
	//Antigo C6_SALDO = CASE WHEN SC6.C6_QTDENT > SC6.C6_QTDVEN THEN 0 ELSE SC6.C6_QTDVEN - SC6.C6_QTDENT END,

	cQuery += "		CASE  "
	cQuery += "		WHEN (SELECT SUM(C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) >= C6_QTDVEN	"
	cQuery += "		THEN 0 	"
	cQuery += "		WHEN (SELECT SUM(C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) = (SELECT SUM(C6_QTDVEN) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO)	"
	cQuery += "		THEN 0	"

	//WHEN SC6.C6_QTDENT >= SC6.C6_QTDVEN THEN 0
...
//Inicio Alterações
//-- Alterado por Davidson Carvalho a pedido do Tiago Gregoletto em 22/01/2023 -  Motivo : Ajuste C9_BLCRED,C9_BLEST,C9_BLWMS.
...
//	Novo 22/01/2023
	cQuery += " CASE "
	cQuery += " WHEN  (SELECT SUM (C6_QTDVEN-C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) = 0 "
	cQuery += "	THEN '10' "
	cQuery += "	ELSE (SELECT TOP 1 C9_BLCRED FROM SC9010 SC9 WHERE SC9.D_E_L_E_T_='' AND SC6.C6_FILIAL = SC9.C9_FILIAL AND SC6.C6_NUM = SC9.C9_PEDIDO AND SC6.C6_ITEM = SC9.C9_ITEM AND SC6.C6_CLI = SC9.C9_CLIENTE AND SC6.C6_LOJA = SC9.C9_LOJA AND SC6.C6_PRODUTO = SC9.C9_PRODUTO ORDER BY SC9.C9_SEQUEN DESC) "
	cQuery += " END  C9_BLCRED, "
...
//	cQuery += "(select top 1 C9_BLEST from SC9010 SC9 where SC9.D_E_L_E_T_='' and SC6.C6_FILIAL = SC9.C9_FILIAL AND SC6.C6_NUM =  SC9.C9_PEDIDO AND SC6.C6_ITEM = SC9.C9_ITEM AND SC6.C6_CLI = SC9.C9_CLIENTE AND SC6.C6_LOJA = SC9.C9_LOJA ORDER BY SC9.C9_SEQUEN DESC) as C9_BLEST, "
// 	Novo 22/01/2023
	cQuery += " CASE "
	cQuery += "	WHEN (SELECT SUM(C6_QTDVEN-C6_QTDENT) FROM SC6010 A WHERE A.D_E_L_E_T_ = '' AND A.C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND SC6.C6_PRODUTO = A.C6_PRODUTO) = 0 "
	cQuery += "	THEN '10' "
	cQuery += "	ELSE (SELECT TOP 1 C9_BLEST FROM SC9010 SC9 WHERE SC9.D_E_L_E_T_='' AND SC6.C6_FILIAL = SC9.C9_FILIAL AND SC6.C6_NUM =  SC9.C9_PEDIDO AND SC6.C6_ITEM = SC9.C9_ITEM AND SC6.C6_CLI = SC9.C9_CLIENTE AND SC6.C6_LOJA = SC9.C9_LOJA AND SC6.C6_PRODUTO = SC9.C9_PRODUTO ORDER BY SC9.C9_SEQUEN DESC) "
	cQuery += " END C9_BLEST, "
...
//	Novo 22/01/2023
	cQuery +=  " (SELECT TOP 1 C9_BLWMS FROM SC9010 SC9 WHERE SC9.D_E_L_E_T_='' AND SC6.C6_FILIAL = SC9.C9_FILIAL AND SC6.C6_NUM =  SC9.C9_PEDIDO AND SC6.C6_ITEM = SC9.C9_ITEM AND SC6.C6_CLI = SC9.C9_CLIENTE AND SC6.C6_LOJA = SC9.C9_LOJA AND SC6.C6_PRODUTO = SC9.C9_PRODUTO ORDER BY SC9.C9_SEQUEN DESC) AS C9_BLWMS "
	cQuery +=  " FROM SC5010  SC5 INNER JOIN  SC6010  SC6 "
	...
	//cQuery +=  " AND C6_SERIE = '6' "
	//cQuery += "  AND SC6.C6_NUM = '103711' " //já estava comentado em 22/01/2023
	cQuery +=  " ORDER BY SC5.C5_FILIAL, SC5.C5_NUM "

//-- Bloco anterior	substituido pelo logo acima
//	cQuery += " FROM (" + RetSQLName("SC5") + " SC5 inner join " + RetSQLName("SC6") + " SC6 "
//	cQuery += " ON SC5.C5_FILIAL = SC6.C6_FILIAL and SC5.C5_NUM = SC6.C6_NUM ) "
//	cQuery += " left outer join  SC9010 SC9 "  						-->já estava comentado em 22/01/2023
//	cQuery += "	ON SC6.C6_FILIAL = SC9.C9_FILIAL AND C6_NUM = C9_PEDIDO AND C6_ITEM = C9_ITEM) --> já estava comentado em 22/01/2023
...
//Fim Alterações 	
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			ElseIf ( cAliasT )->C5_LIBEROK = 'S' .And. Empty(( cAliasT )->C6_NOTA)  .And. ( cAliasT )->C6_STPESTQ != "R" .And. ( cAliasT )->C6_QTDENT = 0
				cStsPv := "Incluido"
			Else
				If Empty(cStsPv)
					cStsPv := "Sem Status"
...
Static Function M010DA0Exp(oProcess,lAuto)
    ...
    // --- Atualiza a tabela 024 preco de custo ---
	 // COMENTADA ALINHA ABAIXO POR IATAN EM 07/05/2024 - CHAMADO #13974
	 //U_MAH0181()
    // --------------------------------------------
...
Static Function M010SB8Exp(oProcess,lAuto)
	...
	Local nAnoDtValidSB8 := SuperGetMv( "MH_ANOSB8",.F.,2050 ) // Ano da maior data de validar dos saldo por lote  SB8 - Saldos por Lote
	Local cAliasT        := GetNextAlias()
	Local cQuery         := ""
	Local cMsg           := ""
	Local cExtracao      := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If  Year( StoD( ( cAliasT )->B8_DTVALID ) ) > nAnoDtValidSB8
				cDtValid := Alltrim("31/12/"+Str(nAnoDtValidSB8)) 
			Else
...
Static Function M010SC7Exp( oProcess,lAuto )
	...
	Local nAnoDtEntrega := SuperGetMv( "MH_ANOENTR",.F.,2050 ) // Ano da maior data de entrega do Pedido de compras	 SC7 - Ped.Compra / Aut.Entrega
	Local nAnoDtEmissao := SuperGetMv( "MH_ANOEMIS",.F.,2050 ) // Ano da maior data de emissão do Pedido de compras  SC7 - Ped.Compra / Aut.Entrega
	Local cAliasT       := GetNextAlias()
	Local cQuery        := ""
	Local cMsg          := ""
	Local cExtracao     := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If  Year( StoD( ( cAliasT )->C7_DATPRF ) ) > nAnoDtEntrega
				cDtPRF := Alltrim("31/12/"+Str(nAnoDtEntrega)) 
			Else
				cDtPRF := If( Empty( ( cAliasT )->C7_DATPRF )	,"",AllTrim( DtoC( StoD( ( cAliasT )->C7_DATPRF ) ) ) ) 
			EndIf      
			
			If  Year( StoD( ( cAliasT )->C7_EMISSAO ) ) > nAnoDtEmissao
				cData := Alltrim("31/12/"+Str(nAnoDtEmissao)) 
			Else
...
Static Function M010AA3Exp(oProcess,lAuto)
	...
	Local nAnoDtVenda      := SuperGetMv( "MH_ANOVEND",.F.,2050 ) // Ano da maior data de venda       AA3 - Base de Atendimento
	Local nAnoDtInstalacao := SuperGetMv( "MH_ANOINST",.F.,2050 ) // Ano da maior data da Instalacao  AA3 - Base de Atendimento
	Local nAnoDtGarantia   := SuperGetMv( "MH_ANOGARA",.F.,2050 ) // Ano da maior data de Garantia    AA3 - Base de Atendimento
	Local cAliasT          := GetNextAlias()
	Local cQuery           := ""
	Local cMsg             := ""
	Local cExtracao        := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If  Year( StoD( ( cAliasT )->AA3_DTVEND ) ) > nAnoDtVenda
				cDtVend := Alltrim("31/12/"+Str(nAnoDtVenda)) 
			Else
				cDtVend := If( Empty( ( cAliasT )->AA3_DTVEND )	,"",AllTrim( DtoC( StoD( ( cAliasT )->AA3_DTVEND ) ) ) )
			EndIf

			If  Year( StoD( ( cAliasT )->AA3_DTINST ) ) > nAnoDtInstalacao
				cDtInst := Alltrim("31/12/"+Str(nAnoDtInstalacao)) 
			Else
				cDtInst := If( Empty( ( cAliasT )->AA3_DTINST )	,"",AllTrim( DtoC( StoD( ( cAliasT )->AA3_DTINST ) ) ) )
			EndIf
						
			If  Year( StoD( ( cAliasT )->AA3_DTGAR ) ) > nAnoDtGarantia
				cDtGar := Alltrim("31/12/"+Str(nAnoDtGarantia)) 
			Else
...
Static Function M010SCPExp( oProcess,lAuto )
	...
	Local nAnoDtPrevista := SuperGetMv( "MH_ANOPREV",.F.,2050 ) // Ano da maior data prev. da Necessidade   SCP - Solicitacoes ao Armazem
	Local nAnoDtEmissao  := SuperGetMv( "MH_ANOSCP",.F.,2050 ) // Ano da maior data de Emissao da SA	    SCP - Solicitacoes ao Armazem
	Local cAliasT        := GetNextAlias()
	Local cQuery         := ""
	Local cMsg           := ""
	Local cExtracao      := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If  Year( StoD( ( cAliasT )->CP_DATPRF ) ) > nAnoDtPrevista
				cDtPrf := Alltrim("31/12/"+Str(nAnoDtPrevista)) 
			Else
				cDtPrf := If( Empty( ( cAliasT )->CP_DATPRF ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->CP_DATPRF ) ) ) )
			EndIf
			
			If  Year( StoD( ( cAliasT )->CP_EMISSAO ) ) > nAnoDtEmissao
				cDtEmiss := Alltrim("31/12/"+Str(nAnoDtEmissao)) 
			Else
...
Static Function M010AB1Exp(oProcess,lAuto)
	...
	Local nAnoEmissChamado := SuperGetMv( "MH_ANOCHAM",.F.,2050 ) // Ano da maior data de emissao do Chamado		AB1 - Chamado Tecnico
	Local nAnoDtBxItem     := SuperGetMv( "MH_ANOITEN",.F.,2050 ) // Ano da maior data/hora da Ultima Baixa Iten  	AB2 - Itens do Chamado Tecnico
	Local cAliasT          := GetNextAlias()
	Local cQuery           := ""
	Local cMsg             := ""
	Local cExtracao        := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If Year( StoD( ( cAliasT )->AB1_EMISSA ) ) > nAnoEmissChamado
				cDtEmiss1 := Alltrim("31/12/"+Str(nAnoEmissChamado)) 
			Else
				cDtEmiss1 := If( Empty( ( cAliasT )->AB1_EMISSA ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB1_EMISSA ) ) ) )
			EndIf

			If Year( StoD( ( cAliasT )->AB2_BXHORA ) ) > nAnoDtBxItem
				cDtEmiss2 := Alltrim("31/12/"+Str(nAnoDtBxItem))
			Else
...
Static Function M010AB3Exp(oProcess,lAuto)
	...
	Local nAnoEmissOrc   := SuperGetMv( "MH_ANORCAM",.F.,2050 ) // Ano da maior data/hora de emissão Orcamento	AB3 - Orcamento Tecnico
	Local nAnoDtVenc1    := SuperGetMv( "MH_ANOVEN1",.F.,2050 ) // Ano da maior data vencimento da Parc. 1	    AB3 - Orcamento Tecnico	
	Local nAnoDtVenc2    := SuperGetMv( "MH_ANOVEN2",.F.,2050 ) // Ano da maior data vencimento da Parc. 2	    AB3 - Orcamento Tecnico	
	Local nAnoDtVenc3    := SuperGetMv( "MH_ANOVEN3",.F.,2050 ) // Ano da maior data vencimento da Parc. 3	    AB3 - Orcamento Tecnico	
	Local nAnoDtVenc4    := SuperGetMv( "MH_ANOVEN4",.F.,2050 ) // Ano da maior data vencimento da Parc. 4	    AB3 - Orcamento Tecnico	
	Local nAnoDtUltBaixa := SuperGetMv( "MH_ANOTECN",.F.,2050 ) // Ano da maior data/hora de Ultima Baixa			AB4 - Itens do Orcamento Tecnico
	Local cAliasT        := GetNextAlias()
	Local cQuery         := ""
	Local cMsg           := ""
	Local cExtracao      := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If Year( StoD( ( cAliasT )->AB3_EMISSA ) ) > nAnoEmissOrc
				cDtEmiss1 := Alltrim("31/12/"+Str(nAnoEmissOrc))
			Else
				cDtEmiss1 := If( Empty( ( cAliasT )->AB3_EMISSA ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB3_EMISSA ) ) ) )
			EndIf				

			If Year( StoD( ( cAliasT )->AB3_DATA1 ) ) > nAnoDtVenc1
				cData1 := Alltrim("31/12/"+Str(nAnoDtVenc1)) 
			Else
				cData1 := If( Empty( ( cAliasT )->AB3_DATA1 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB3_DATA1 ) ) ) )
			EndIf

			If Year( StoD( ( cAliasT )->AB3_DATA2 ) ) > nAnoDtVenc2
				cData2 := Alltrim("31/12/"+Str(nAnoDtVenc2)) 
			Else
				cData2 := If( Empty( ( cAliasT )->AB3_DATA2 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB3_DATA2 ) ) ) )
			EndIf

			If Year( StoD( ( cAliasT )->AB3_DATA3 ) ) > nAnoDtVenc3
				cData3 := Alltrim("31/12/"+Str(nAnoDtVenc3))
			Else
				cData3 := If( Empty( ( cAliasT )->AB3_DATA3 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB3_DATA3 ) ) ) )
			EndIf

			If Year( StoD( ( cAliasT )->AB3_DATA4 ) ) > nAnoDtVenc4
				cData4 := Alltrim("31/12/"+Str(nAnoDtVenc4))
			Else
				cData4 := If( Empty( ( cAliasT )->AB3_DATA4 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB3_DATA4 ) ) ) )
			EndIf
																	
			If Year( StoD( ( cAliasT )->AB4_BXHORA ) ) > nAnoDtUltBaixa
				cBxData := Alltrim("31/12/"+Str(nAnoDtUltBaixa)) 
			Else
...
Static Function M010ABFEXP(oProcess,lAuto)
	...
	Local nAnoDtEmisRepar := SuperGetMv( "MH_ANOREPA",.F.,2050 ) // Ano da maior data de emissão 	ABF - Repair Center
	Local cAliasT         := GetNextAlias()
	Local cQuery          := ""
	Local cMsg            := ""
	Local cExtracao       := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If Year( StoD( ( cAliasT )->ABF_EMISSA ) ) > nAnoDtEmisRepar
				cDtEmiss := Alltrim("31/12/"+Str(nAnoDtEmisRepar)) 
			Else
...
Static Function M010AB6Exp(oProcess,lAuto)
	...
	Local nAnoEmisOrdem   := SuperGetMv( "MH_ANORDEM",.F.,2050 ) //Ano da maior data de emissão	AB6 - Ordens de Servicos
	Local nAnoDtVencParc1 := SuperGetMv( "MH_ANOPAR1",.F.,2050 ) //Ano da maior data vencimento da Parc. 1	AB6 - Ordens de Servicos
	Local nAnoDtVencParc2 := SuperGetMv( "MH_ANOPAR2",.F.,2050 ) //Ano da maior data vencimento da Parc. 2	AB6 - Ordens de Servicos
	Local nAnoDtVencParc3 := SuperGetMv( "MH_ANOPAR3",.F.,2050 ) //Ano da maior data vencimento da Parc. 3	AB6 - Ordens de Servicos
	Local nAnoDtVencParc4 := SuperGetMv( "MH_ANOPAR4",.F.,2050 ) //Ano da maior data vencimento da Parc. 4	AB6 - Ordens de Servicos
	Local cAliasT         := GetNextAlias()
	Local cQuery          := ""
	Local cMsg            := ""
	Local cExtracao       := DtoC( Date() ) + " - " + Time()
	...
	If ( cAliasT )->( !Eof() )
		...
		While ( cAliasT )->( !Eof() )
			...
			If Year( StoD( ( cAliasT )->AB6_EMISSA ) ) > nAnoEmisOrdem
				cDtEmiss := Alltrim("31/12/"+Str(nAnoEmisOrdem)) 
			Else
				cDtEmiss := If( Empty( ( cAliasT )->AB6_EMISSA ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB6_EMISSA ) ) ) )
			EndIf
			
			If Year( StoD( ( cAliasT )->AB6_DATA1 ) ) > nAnoDtVencParc1
				cData1 := Alltrim("31/12/"+Str(nAnoDtVencParc1)) 
			Else
				cData1 := If( Empty( ( cAliasT )->AB6_DATA1 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB6_DATA1 ) ) ) )
			EndIf							
			
			If Year( StoD( ( cAliasT )->AB6_DATA2 ) ) > nAnoDtVencParc2
				cData2 := Alltrim("31/12/"+Str(nAnoDtVencParc2)) 
			Else
				cData2 := If( Empty( ( cAliasT )->AB6_DATA2 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB6_DATA2 ) ) ) )
			EndIf			

			If Year( StoD( ( cAliasT )->AB6_DATA3 ) ) > nAnoDtVencParc3
				cData3 := Alltrim("31/12/"+Str(nAnoDtVencParc3)) 
			Else
				cData3 := If( Empty( ( cAliasT )->AB6_DATA3 ) ,"",AllTrim( DtoC( StoD( ( cAliasT )->AB6_DATA3 ) ) ) )
			EndIf			

			If Year( StoD( ( cAliasT )->AB6_DATA4 ) ) > nAnoDtVencParc4
				cData4 := Alltrim("31/12/"+Str(nAnoDtVencParc4))
			Else
...
Static Function M010CriaSX1(cPerg)
	...
	aAdd( aPerg, { "06", "Exportar Cliente?"		 		, "mv_ch5", "N", 01 , 0 ,"MV_PAR06","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "07", "Exportar Vendedores?"	 			, "mv_ch6", "N", 01 , 0 ,"MV_PAR07","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "08", "Exportar Produtos?"		 		, "mv_ch7", "N", 01 , 0 ,"MV_PAR08","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "09", "Exportar Pedido Venda?"	 		, "mv_ch8", "N", 01 , 0 ,"MV_PAR09","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "10", "Exportar Fornecedor?"	 			, "mv_ch9", "N", 01 , 0 ,"MV_PAR10","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "11", "Exportar CRM?"			 		, "mv_cha", "N", 01 , 0 ,"MV_PAR11","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "12", "Exportar Status Pedido?"	 		, "mv_chb", "N", 01 , 0 ,"MV_PAR12","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "13", "Exportar Faturamento?"	 		, "mv_chc", "N", 01 , 0 ,"MV_PAR13","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "14", "Exportar Contas Receber?" 		, "mv_chd", "N", 01 , 0 ,"MV_PAR14","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "15", "Exportar Contas a Pagar?" 		, "mv_che", "N", 01 , 0 ,"MV_PAR15","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "16", "Exportar Mov.Financeiro?" 		, "mv_chf", "N", 01 , 0 ,"MV_PAR16","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "17", "Exportar Lanc.Contabil?"  		, "mv_chg", "N", 01 , 0 ,"MV_PAR17","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "18", "Exportar Plano de Contas?"		, "mv_chh", "N", 01 , 0 ,"MV_PAR18","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "19", "Exportar Visoes Gerencias?"  		, "mv_chi", "N", 01 , 0 ,"MV_PAR19","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "20", "Exportar Orçamentos?"  			, "mv_chj", "N", 01 , 0 ,"MV_PAR20","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )	
	aAdd( aPerg, { "21", "Exportar TES?" 		 			, "mv_chk", "N", 01 , 0 ,"MV_PAR21","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "22", "Exportar Centro de Custos?" 		, "mv_chl", "N", 01 , 0 ,"MV_PAR22","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "23", "Exportar Meta de Vendas?" 		, "mv_chm", "N", 01 , 0 ,"MV_PAR23","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "24", "Exportar Tabela de Preços?" 		, "mv_chn", "N", 01 , 0 ,"MV_PAR24","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "25", "Exportar Estabelecimentos?" 		, "mv_cho", "N", 01 , 0 ,"MV_PAR25","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "26", "Exportar Documento Entrada?"		, "mv_chp", "N", 01 , 0 ,"MV_PAR26","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "27", "Exportar Saldo Fisico/Financ?" 	, "mv_chq", "N", 01 , 0 ,"MV_PAR27","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "28", "Exportar Saldo por Lote?" 		, "mv_chr", "N", 01 , 0 ,"MV_PAR28","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "29", "Exportar Poder de Terceiros?" 	, "mv_chs", "N", 01 , 0 ,"MV_PAR29","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "30", "Exportar Tipo de Movimentação?" 	, "mv_cht", "N", 01 , 0 ,"MV_PAR30","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "31", "Exportar Tabela de Armazens?" 	, "mv_chu", "N", 01 , 0 ,"MV_PAR31","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "32", "Exportar Movimentacoes Internas?"	, "mv_chv", "N", 01 , 0 ,"MV_PAR32","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "33", "Exportar Pedido de Compra?"		, "mv_chx", "N", 01 , 0 ,"MV_PAR33","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "34", "Exportar Natureza de Operação?"	, "mv_chy", "N", 01 , 0 ,"MV_PAR34","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "35", "Exportar Gestão de Serviços?"		, "mv_chz", "N", 01 , 0 ,"MV_PAR35","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "36", "Exportar Solicitação ao Armazem?"	, "mv_ch0", "N", 01 , 0 ,"MV_PAR36","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "37", "Exportar Tabela de Técnicos?"		, "mv_ch1", "N", 01 , 0 ,"MV_PAR37","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "38", "Exportar Chamado Tecnico?"		, "mv_ch2", "N", 01 , 0 ,"MV_PAR38","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "39", "Exportar Orçamento Técnico?"		, "mv_ch3", "N", 01 , 0 ,"MV_PAR39","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "40", "Exportar Requisição de OS?"		, "mv_ch4", "N", 01 , 0 ,"MV_PAR40","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "41", "Exportar Ordem de Serviço?"		, "mv_ch5", "N", 01 , 0 ,"MV_PAR41","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "42", "Exportar Moedas?"					, "mv_ch6", "N", 01 , 0 ,"MV_PAR42","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "43", "Exportar Comodatos?"				, "mv_ch7", "N", 01 , 0 ,"MV_PAR43","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "44", "Exportar Fechamento Estoque?"		, "mv_ch8", "N", 01 , 0 ,"MV_PAR44","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "45", "Exportar App Inventario?"		    , "mv_ch9", "N", 01 , 0 ,"MV_PAR45","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	aAdd( aPerg, { "46", "Exportar Matriz Reposicao"		, "mv_chw", "N", 01 , 0 ,"MV_PAR46","C" ," ", "Sim","Não"," " ," "	," "  ,"" } )
	...
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da tabela ", " de Movimentação Financeira (SE5)" })	
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da tabela ", " de Lançamentos Contabeis (CT2)" })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da tabela ", " de Plano de Contas (CT1)" })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da tabela ", " de Visões Gerenciais (CTS)" })	
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da tabela ", " de Orçamentos (CV1)." })	
	...
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da  "		 	, " Tabela de Preços (DA0/DA1)." })
	...
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Tipos de Movimentação (SF5)." })
	...
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Naturezas de Operação (SED)." })	
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Gestão de Serviços (AA3)." })	
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Solicitação ao Armazem (SCP)." })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Técnicos (AA1)." })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Chamado Técnico (AB1/AB2)." })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Orçamento Técnico (AB3/AB4/AB5)." })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Requisição de OS (ABF/ABG)." })
	aAdd(aHelp,{"Informe se deseja ou nao rodar ", " a exportacao da Tabela "	, " de Ordem de Serviço (AB6/AB7/AB8)." })
