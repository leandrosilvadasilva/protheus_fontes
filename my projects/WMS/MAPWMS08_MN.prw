		...
		If SC9->(DbSeek(cFilPed+cPedido))
			While SC9->(!Eof()) .And. SC9->(C9_FILIAL+C9_PEDIDO) == cFilPed+cPedido
				IF EMPTY(ALLTRIM(SC9->C9_NFISCAL)) // IATAN EM 25/08/2022
					...
					If  MaisNegociosLigado()   .and.   NaoVazio( SC9->C9_TICKETC )

						aAdd( aSC9MaisNegocios, SC9->C9_FILIAL  )
						aAdd( aSC9MaisNegocios, SC9->C9_PEDIDO  )
						aAdd( aSC9MaisNegocios, SC9->C9_TICKETC )
						aAdd( aSC9MaisNegocios, SC9->C9_ITEM    )
						aAdd( aSC9MaisNegocios, SC9->C9_SEQUEN  )
						aAdd( aSC9MaisNegocios, SC9->C9_PRODUTO )																		
						aAdd( aSC9MaisNegocios, SC9->(Recno())  )
			...
			VarInfo( "aSC9MaisNegocios", aSC9MaisNegocios )
		Endif
	...
	For nI := 1 To Len(aParam)
		...
		IF EMPTY(ALLTRIM(SC9->C9_NFISCAL)) .AND. EMPTY(ALLTRIM(SC9->C9_BLCRED))  
			...
			*/
			...
			If  MaisNegociosLigado() 

				// X2_UNICO: C9_FILIAL+C9_PEDIDO+C9_ITEM+C9_SEQUEN+C9_PRODUTO
				cAliasSC9   := GetNextAlias()
				cSC9Filial  := SC9->C9_FILIAL
				cSC9Pedido  := SC9->C9_PEDIDO
				cSC9Item    := SC9->C9_ITEM
				cSC9Sequen  := SC9->C9_SEQUEN
				cSC9Produto := SC9->C9_PRODUTO
				
				BEGINSQL Alias cAliasSC9

					SELECT SC9.C9_FILIAL, SC9.C9_PEDIDO, SC9.C9_ITEM, SC9.C9_SEQUEN, SC9.C9_PRODUTO, SC9.R_E_C_N_O_, SC9.D_E_L_E_T_, SC9.C9_TICKETC
					WHERE 1=1
					AND SC9.D_E_L_E_T_ = '*'
					AND SC9.C9_FILIAL  = %Exp:cSC9Filial%
					AND SC9.C9_PEDIDO  = %Exp:cSC9Pedido% 
					AND SC9.C9_ITEM    = %Exp:cSC9Item%
					AND SC9.C9_SEQUEN  = %Exp:cSC9Sequen%
					AND SC9.C9_PRODUTO = %Exp:cSC9Produto%
				...
				If (cAliasSC9)->(!Eof())
					...
					RecLock("SC9", .F.)
					SC9->C9_TICKETC := (cAliasSC9)->C9_TICKETC
					SC9->(MsUnLock("SC9"))
				EndIf
			EndIf
		...
		while !SC9->( EOF() ) .AND. SC9->C9_FILIAL + SC9->C9_PEDIDO == filialPedido + numeroPedido 
			...
			IF ALLTRIM(STR(SC9->(RECNO()))) $ listRecnoSC9 .AND. EMPTY(ALLTRIM(SC9->C9_BLEST)) .AND. EMPTY(ALLTRIM(SC9->C9_BLCRED)) .AND. EMPTY(ALLTRIM(SC9->C9_NFISCAL))
				...
				IF NCOUNT == NNUMITEM
					...
					//IATAN EM 26/09/2023
					IF ALLTRIM(serie) == "6"
						SLEEP(7000) // PARA O PROCESSO POR 7 SEGUNDOS SOMENTE NO CONTEXTO DO OPME
					ENDIF
...
Static Function MaisNegociosLigado() 
Local lRetorno := .T.
